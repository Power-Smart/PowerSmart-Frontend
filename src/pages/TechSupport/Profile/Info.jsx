...
const Info = () => {
    ...
    useEffect(() => {
        setData({
            ...
            tel_no: techSupportInfo?.tel_no || [],
        });
    }, [techSupportInfo, user]);
    ...
    const updateTelNo = (e, index) => {
        e.preventDefault();
        const newTelNoArr = [...data.tel_no];
        newTelNoArr[index] = e.target.value;
            ...
            <div className='profile-details'>
                {!editToggle && (
                    <div className='flex w-full justify-end'>
                        <div
                            className='p-2 bg-white/10 cursor-pointer rounded-md'
                    ...
                    <div className='mt-8'>
                        ...
                        {data.tel_no.map((tel, index) => (
                            <FormGroup key={index}>
                                ...
                                />
                                {data.tel_no.length > 1 && (
                                    <button
                                        type='button'
                                        className='absolute top-0 right-0'
                                        onClick={() => {
                                            const updatedTelNoArr = data.tel_no.filter((no) => no !== tel);
                                            setData({
                                                ...data,
                                                tel_no: updatedTelNoArr,
                                            });
                                        }}
                                    >
                                        X
                        ...
                        {data.tel_no.length < 3 && (
                            <button
                                type='button'
                                className='text-blue-500 p-2 cursor-pointer'
                                onClick={() => {
                                    setData({ ...data, tel_no: [...data.tel_no, ''] });
                                }}
                            >
                                + Add Tel No
                            </button>
                    ...
                    <div className='py-3'>
                        ...
                        <FormGroup>
                            <div className='flex flex-wrap space-x-4 text-base lg:text-lg md:text-sm rounded-xl flex-col'>
                                <label className='px-3 py-1 rounded-xl text-blue-400'>
                                    Telephone Numbers :
                                </label>
                                {data.tel_no.map((tel, index) => (
                                    <label
                                        key={tel}
                                        className='m-1 px-3 py-1 rounded-xl bg-gray-900'
                                    >
                                        {tel}
                                    </label>
                                ))}
